...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  world_cup_script="$(shell_quote "$SCRIPT_DIR/apply-world-cup-preset-macos.sh")"
  ...
  write_launcher "$HOME/Desktop/Codex World Cup - Spain.command" "exec $world_cup_script spain"
  write_launcher "$HOME/Desktop/Codex World Cup - Argentina.command" "exec $world_cup_script argentina"
fi
